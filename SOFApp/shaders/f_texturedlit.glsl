...
// normals and normal mapping
	...
	n = normalize(n); 
    vec3 e = normalize(cameraPos - pixelPos);
    vec3 reflectE = reflect(-e , n);
    vec4 envMapColor = texture(cubeMapSampler, reflectE);
...
// four point lights
    ...
    finalColor.a = 1.0f;
    finalColor = finalColor*(1.0f-fEnvMap) + envMapColor*fEnvMap;
}