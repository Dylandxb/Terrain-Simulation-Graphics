	...
	void CreateGeometry(int tesselation)
	{
		...
		for (int i = 0; i < tesselation; i++)
		{
			...
			for (int j = 0; j < tesselation; j++)
			{
				...
				vertexPos[i][j] = glm::vec3(x, y, z);								//Sphere Normal
				...
				if (y > -0.999f && y < 0.999f)
					...
					vertexTangent[i][j] = glm::cross(vertexPos[i][j], glm::vec3(1, 0, 0));
		...
		for (int i = 0; i < tesselation - 1; i++)
		{
			for (int j = 0; j < tesselation - 1; j++)
			{
				...
				*/
				...
				//tangents.push_back(vertexTangent[i][j]); tangents.push_back(vertexTangent[i][j]); tangents.push_back(vertexTangent[i][j]);
